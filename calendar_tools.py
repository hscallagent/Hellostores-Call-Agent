...
async def get_business_hours_from_calcom() -> dict:
    """Fetch real working hours live from Cal.com event type schedule."""
    creds = get_cal_creds()
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            resp = await client.get(
                f"{CAL_BASE}/event-types/{creds['event_id']}",
                params={"apiKey": creds["api_key"]},
            )
        data = resp.json()
        schedule     = data.get("eventType", {}).get("schedule", {})
        availability = schedule.get("availability", [])
        timezone_str = schedule.get("timeZone", "Asia/Kolkata")
        ...
        for entry in availability:
            for day_num in entry.get("days", []):
                day_name = days_map.get(day_num, str(day_num))
                hours[day_name] = {
                    "open":  entry.get("startTime", ""),
                    "close": entry.get("endTime", ""),
                }

        logger.info(f"[CAL] Business hours fetched: {hours}")
        return {"timezone": timezone_str, "hours": hours}

    except Exception as e:
        logger.error(f"[CAL] get_business_hours_from_calcom error: {e}")
        return {}
...
async def get_available_slots(date_str: str) -> list:
    ...
    if gcal_id and os.path.exists(gcal_creds):
        try:
            return await _get_slots_gcal(date_str, gcal_id, gcal_creds)
        except Exception as e:
            logger.warning(f"[GCAL] Falling back to Cal.com: {e}")

    # Default: Cal.com
    return await _get_slots_calcom(date_str)


async def _get_slots_calcom(date_str: str) -> list:
    creds = get_cal_creds()
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            resp = await client.get(
        ...
        raw_slots = resp.json().get("slots", {}).get(date_str, [])
        slots = []
        ...
        for s in raw_slots:
            ...
            slots.append({
                "time":     dt_utc.strftime("%Y-%m-%dT%H:%M:%S.000Z"),   # UTC key for comparison
                "time_ist": dt_ist.strftime("%Y-%m-%dT%H:%M:%S+05:30"),  # IST for booking
...
async def _get_slots_gcal(date_str: str, calendar_id: str, creds_file: str) -> list:
    """
    ...
    # Use Cal.com business hours dynamically
    bh = {}
    try:
        bh = await get_business_hours_from_calcom()
    except Exception:
        pass
    dt_check  = datetime.strptime(date_str, "%Y-%m-%d")
    day_name  = dt_check.strftime("%A")
    day_hours = bh.get("hours", {}).get(day_name)
    open_t    = day_hours["open"]  if day_hours else "09:00"
    close_t   = day_hours["close"] if day_hours else "17:00"
    day_start = ist.localize(datetime.strptime(f"{date_str} {open_t}", "%Y-%m-%d %H:%M"))
    day_end   = ist.localize(datetime.strptime(f"{date_str} {close_t}", "%Y-%m-%d %H:%M"))
    ...
    while slot < day_end:
        ...
        if not is_busy:
            free_slots.append({
                "time":  slot.isoformat(),
                "time_ist": slot.strftime("%Y-%m-%dT%H:%M:%S+05:30"),
...
async def _create_booking_calcom(
    ...
    payload = {
        ...
        "responses": {
            "name":  caller_name,
            "email": notes if "@" in (notes or "") else "guest@voiceagent.placeholder",
        },
    }
    try:
        async with httpx.AsyncClient(timeout=8.0) as client:
            resp = await client.post(
                f"https://api.cal.com/v1/bookings?apiKey={creds['api_key']}",
                headers={"Content-Type": "application/json"},
                json=payload,
            )
        
        # Log the raw response for debugging
        logger.info(f"[CAL] Booking response {resp.status_code}: {resp.text[:300]}")
        ...
        if resp.status_code not in (200, 201):
            ...
            # Detect double-booking specifically
            resp_text = resp.text.lower()
            if any(k in resp_text for k in ("already", "conflict", "unavailable", "taken", "booked")):
                msg = "That slot is already booked."
            else:
                msg = resp_json.get("message", resp.text[:200])
            
            logger.error(f"[CAL] Booking failed {resp.status_code}: {msg}")
