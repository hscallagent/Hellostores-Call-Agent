...
class AgentTools(llm.ToolContext):
    ...
    @llm.function_tool(description="Get available appointment slots for a given date. Call this when the caller asks about available times.")
    async def check_available_slots(
        self,
        date: Annotated[str, "Date in YYYY-MM-DD format, e.g. '2026-03-09'"],
    ) -> str:
        ...
        # Group into morning / afternoon / evening for voice-friendly output
        morning, afternoon, evening = [], [], []
        for s in slots:
            label = s.get("label", "")
            try:
                hour = int(s.get("label","12:00 AM").split(":")[0])
                is_pm = "PM" in label
                h24 = hour + 12 if is_pm and hour != 12 else (0 if not is_pm and hour == 12 else hour)
            except Exception:
                h24 = 12
            if h24 < 12:
    ...
    async def save_booking_intent(
        ...
        caller_phone: Annotated[str, "Phone number of the caller. If unknown, use 'not provided'"],
        caller_email: Annotated[str, "Email address of the caller e.g. 'abc@gmail.com'. If not provided, use empty string."] = "",
        notes:        Annotated[str, "Any additional notes about the booking. If none, use empty string."] = "",
    ) -> str:
        logger.info(f"[TOOL] save_booking_intent: {caller_name} at {start_time} email={caller_email}")
        try:
            # ── Pre-validate: check slot is still available ───────────────
            try:
                from datetime import timezone, timedelta
                _IST = timezone(timedelta(hours=5, minutes=30))
                _dt = datetime.fromisoformat(start_time.replace("Z", "+00:00"))
                _date_str = _dt.astimezone(_IST).strftime("%Y-%m-%d")
                _time_label = _dt.astimezone(_IST).strftime("%I:%M %p").lstrip("0")

                available_slots = await get_available_slots(_date_str)
                available_labels = [s.get("label", "") for s in available_slots]

                # Compare in IST directly — no UTC conversion needed
                _dt_ist_str = _dt.astimezone(_IST).strftime("%Y-%m-%dT%H:%M:%S+05:30")
                available_ist_times = [s.get("time_ist", "") for s in available_slots]
                slot_free = _dt_ist_str in available_ist_times

                if not slot_free:
                    logger.warning(f"[TOOL] save_booking_intent: slot {start_time} already taken — rejecting")
                    return (
                        f"Sorry, the {_time_label} slot on {_date_str} is no longer available — "
                        f"it may have just been booked. "
                        f"Available slots are: {', '.join(available_labels[:5])}. "
                        f"Please ask the caller to choose another time."
                    )
            except Exception as val_err:
                logger.warning(f"[TOOL] Pre-validation failed (proceeding anyway): {val_err}")
    ...
    @llm.function_tool(description="Check business operating hours for any day of the week, or whether we are currently open.")
    async def get_business_hours(self) -> str:
        ...
        # Build full weekly schedule
        schedule_lines = []
        for day_num in range(7):
            day_name, open_t, close_t = hours[day_num]
            if open_t is None:
                schedule_lines.append(f"  {day_name}: Closed")
            else:
                schedule_lines.append(f"  {day_name}: {_fmt(open_t)} – {_fmt(close_t)} IST")

        # Today's status
        today_name, open_t, close_t = hours[now.weekday()]
        if open_t is None:
            today_status = f"Today ({today_name}): CLOSED"
        elif open_t <= current_time <= close_t:
            today_status = f"Today ({today_name}): OPEN now ({_fmt(open_t)} – {_fmt(close_t)} IST)"
        else:
            today_status = f"Today ({today_name}): CLOSED now (hours: {_fmt(open_t)} – {_fmt(close_t)} IST)"

        return f"{today_status}\n\nFull weekly schedule:\n" + "\n".join(schedule_lines)
...
async def entrypoint(ctx: JobContext):
    ...
    for identity, participant in ctx.room.remote_participants.items():
        ...
        if participant.name and participant.name not in ("", "Caller", "Unknown") and not participant.name.startswith("Phone "):
    ...
    async def unified_shutdown_hook(shutdown_ctx: JobContext):
        ...
        if _shutdown_called:
            logger.debug("[SHUTDOWN] Already ran — skipping duplicate.")
            return
        _shutdown_called = True
        logger.info("[SHUTDOWN] Sequence started.")

        duration = int((datetime.now(pytz.timezone("Asia/Kolkata")) - call_start_time).total_seconds())
        ...
        if agent_tools.booking_intent:
            ...
            result = await async_create_booking(
                ...
                caller_phone=caller_phone,
                notes=intent.get("caller_email") or intent.get("notes") or "",
            )
            if result.get("success"):
                from datetime import timezone, timedelta
                _IST    = timezone(timedelta(hours=5, minutes=30))
                _dt     = datetime.fromisoformat(intent["start_time"].replace("Z", "+00:00"))
                _ist_str = _dt.astimezone(_IST).strftime("%d-%m-%Y %I:%M %p IST")
                notify_booking_confirmed(
                    caller_name=intent["caller_name"],
                    caller_phone=caller_phone,
                    booking_time_iso=_ist_str,
                ...
                # ── NEW: notify failure on Telegram too ──────────────────────
                err_msg = result.get('message', 'Unknown error')
                logger.error(f"[SHUTDOWN] Booking failed: {err_msg}")
                notify_call_no_booking(
                    caller_name=intent["caller_name"],
                    caller_phone=caller_phone,
                    call_summary=f"Booking FAILED: {err_msg}. Caller wanted {intent['start_time']}.",
                    tts_voice=tts_voice,
                    duration_seconds=duration,
                )
                booking_status_msg = f"Booking Failed: {err_msg}"
        else:
            notify_call_no_booking(
