...
try:
    ...
    def _c(name, desc):
        return REGISTRY._names_to_collectors.get(name) or Counter(name, desc)
    def _h(name, desc, buckets):
        return REGISTRY._names_to_collectors.get(name + "_bucket") or Histogram(name, desc, buckets=buckets)
    def _g(name, desc):
        return REGISTRY._names_to_collectors.get(name) or Gauge(name, desc)

    _voice_calls_total   = _c("voice_calls_total",            "Total calls handled by the agent")
    _voice_calls_booked  = _c("voice_calls_booked_total",     "Calls that resulted in a booking")
    _voice_call_duration = _h("voice_call_duration_seconds",  "Call duration in seconds",
                               buckets=[10, 30, 60, 120, 300, 600, 1200])
    _voice_calls_active  = _g("voice_calls_active",           "Currently active calls")
...
function renderLangGrid() {{
  ...
  grid.innerHTML = Object.entries(LANG_PRESETS).map(([id, p]) => `
    ...
    " onmouseover="this.style.borderColor='${{p.color}}'" onmouseout="this.style.borderColor='${{id===currentLangPreset?p.color:'#2a3448'}}'">
